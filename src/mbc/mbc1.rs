...
impl MemoryBus for MBC1 {
    ...
    fn read_byte(&self, address: u16) -> u8 {
        match address {
            ...
            0x4000 ..= 0x7FFF => self.rom[(self.rom_bank * 16_384) + address as usize - 16_384],
            // A000-BFFF - RAM Bank 00-03, if any (Read/Write)
            0xA000 ..= 0xBFFF => {
                if self.ram_enable { 
                    self.ram[(self.ram_bank * 8_192) + address as usize - 40_960] 
                } else { 0 }
            },
            _ => 0,
        }
    ...
    fn write_byte(&mut self, address: u16, b: u8) {
        match address {
            ...
            0x4000 ..= 0x5FFF => self.rom_bank = self.rom_bank & 0b11111 | (((b as usize) & 0b11) << 5),
            // Banking Mode Select (Write Only)
            0x6000 ..= 0x7FFF => self.mode = b == 0,
            _ => {},
        }
    }
}