...
pub struct GPU {
    ...
    // Raw pixel data, each pixel one of 4 grey shades as 0xRRGGBB.
...
impl GPU {
    ...
    fn switch_mode(&mut self, mode: Mode) {
        ...
        let interrupt = match self.stat.mode {
            ...
            Mode::VBlank => {
                self.intf
                    .borrow_mut()
                    .set_interrupt(InterruptSource::VBlank);
                self.updated = true;
    ...
    fn render_bg(&mut self) {
        ...
        for x in 0..SCREEN_WIDTH as u8 {
            ...
            let colour_num = usize::from(tile_data[0] & (1 << x_bit) > 0)
                | if tile_data[1] & (1 << x_bit) > 0 {
                    2
                } else {
                    0
                };
    ...
    fn render_sprites(&mut self) {
        ...
        for idx in (0..40).rev() {
            ...
            if line < sprite.y || line >= sprite.y + size {
                continue;
            }
            ...
            for x in 0..8 {
                ...
                if !(0..160).contains(&pix_x) {
                    continue;
                }

                let x_bit = if sprite.x_flip { x } else { 7 - x };

                let colour_idx = usize::from(tile_data[0] & (1 << x_bit) > 0)
                    | if tile_data[1] & (1 << x_bit) > 0 {
                        2
                    } else {
                        0
    ...
    fn clear_screen(&mut self) {
        self.pixels.fill(u32::MAX);
        self.bg_priority.fill(Priority::None);
        self.updated = true;
    }

    fn set_pixel(&mut self, x: usize, colour: u32) {
        self.pixels[(self.ly as usize) * SCREEN_WIDTH + x] = colour;
    }
