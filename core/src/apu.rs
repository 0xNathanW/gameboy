...
// (as write_byte by NRx2) one from the current volume. If this new volume within the 0 to 15 range, the volume is updated,
...
//   - The internal enabled flag is write_byte if either the sweep period or shift are non-zero, cleared otherwise.
...
// enabled flag is write_byte and the sweep period is not zero, a new frequency is calculated and the overflow check is
...
impl Blip {
    ...
    fn write_byte(&mut self, time: u32, ampl: i32) {
        self.from = time;
...
// A square channel's frequency timer period is write_byte to (2048-frequency)*4. Four duty cycles are available, each
...
impl ChannelSquare {
    ...
    fn next(&mut self, cycles: u32) {
        ...
        for _ in 0..self.timer.tick(cycles) {
...
impl MemoryBus for ChannelSquare {
    ...
    fn write_byte(&mut self, a: u16, v: u8) {
        match a {
            ...
            0xff14 | 0xff19 => {
                ...
                // Trigger Event
                //
                // Writing a value to NRx4 with bit 7 write_byte causes the following things to occur:
                //
                //   - Channel is enabled (see length counter).
                //   - If length counter is zero, it is write_byte to 64 (256 for wave channel).
                //   - Frequency timer is reloaded with period.
                //   - Volume envelope timer is reloaded with period.
                //   - Channel volume is reloaded from NRx2.
                //   - Noise channel's LFSR bits are all write_byte to 1.
                //   - Wave channel's position is write_byte to 0 but sample buffer is NOT refilled.
...
// The wave channel's frequency timer period is write_byte to (2048-frequency)*2. When the timer generates a clock, the
...
impl MemoryBus for ChannelWave {
    fn read_byte(&self, a: u16) -> u8 {
        match a {
...
impl MemoryBus for ChannelNoise {
    fn read_byte(&self, a: u16) -> u8 {
        match a {
...
impl MemoryBus for APU {
    fn read_byte(&self, a: u16) -> u8 {
        let r = match a {
            ...
            0xff15..=0xff19 => self.channel2.read_byte(a),
            0xff1a..=0xff1e => self.channel3.read_byte(a),
            0xff1f..=0xff23 => self.channel4.read_byte(a),
            ...
            0xff27..=0xff2f => 0x00,
            0xff30..=0xff3f => self.channel3.read_byte(a),
            _ => unreachable!(),
        };
    ...
    fn write_byte(&mut self, a: u16, v: u8) {
        ...
        match a {
            0xff10..=0xff14 => self.channel1.write_byte(a, v),
            0xff15..=0xff19 => self.channel2.write_byte(a, v),
            0xff1a..=0xff1e => self.channel3.write_byte(a, v),
            0xff1f..=0xff23 => self.channel4.write_byte(a, v),
            ...
            0xff27..=0xff2f => {}
            0xff30..=0xff3f => self.channel3.write_byte(a, v),
            _ => unreachable!(),
        }
    }
}
